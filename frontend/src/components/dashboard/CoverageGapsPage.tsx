...
export function CoverageGapsPage({ overview, policy, onOpenDecision, onOpenRules: openRules, helpers }: CoverageGapsPageProps) {
  ...
  const clusterRows = useMemo(() => {
    ...
    return Array.from(clusters.values()).map((cluster: any) => {
      ...
      return { ...cluster, agents: Array.from(cluster.agents), tools: Array.from(cluster.tools), domains: Array.from(cluster.domains), eventTypes: Array.from(cluster.eventTypes), tags: Array.from(cluster.tags).slice(0, 5), lastSeen, firstSeen, representative, growthHint: cluster.eventCount >= 8 ? `+${Math.min(90, cluster.eventCount * 4)}% in ${timeWindow === '24h' ? '24h' : '3 days'}` : `${cluster.eventCount} recent observations`, suggestedRule: buildSuggestedGapRule(cluster, representative) };
    });
  ...
  const onOpenRules = () => { if (selectedCluster?.suggestedRule) openRules(selectedCluster.suggestedRule); };
