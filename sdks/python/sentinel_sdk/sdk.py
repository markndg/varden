    ...
    if isinstance(content, str):
        ...
        if stripped.startswith('{') or stripped.startswith('['):
            try:
                ...
                return content
    return content
_current_guard: contextvars.ContextVar[SentinelGuard | None] = contextvars.ContextVar('sentinel_guard', default=None)
_current_agent: contextvars.ContextVar[str | None] = contextvars.ContextVar('sentinel_agent', default=None)
_current_workflow: contextvars.ContextVar[str | None] = contextvars.ContextVar('sentinel_workflow', default=None)
_current_lineage: contextvars.ContextVar[dict[str, Any] | None] = contextvars.ContextVar('sentinel_lineage', default=None)
_current_trace_id: contextvars.ContextVar[str | None] = contextvars.ContextVar('sentinel_trace_id', default=None)
_current_parent_event_id: contextvars.ContextVar[int | None] = contextvars.ContextVar('sentinel_parent_event_id', default=None)
...
class SentinelBlockedError(RuntimeError):
...
class SentinelClient:
    ...
    def guard(self, payload: dict[str, Any]) -> GuardResult:
        ...
        if resp.status_code == 403:
            ...
            raise SentinelBlockedError(detail or 'blocked by Sentinel', decision)
        resp.raise_for_status()
...
class SentinelGuard:
    def __init__(
        ...
        self.client = SentinelClient(base_url=base_url, api_key=api_key, bearer_token=bearer_token, timeout=timeout)
    ...
    def activate(self) -> 'SentinelGuard':
        ...
        if inspect.iscoroutinefunction(fn):
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
                ...
                if result and result.blocked and self.mode == 'enforce':
                    raise SentinelBlockedError(f'{tool_name} blocked', result.decision)
                try:
...
def current_guard() -> SentinelGuard | None:
...
class _SentinelImportFinder(importlib.abc.MetaPathFinder):
...
def _install_import_hook() -> None:
    ...
    if _IMPORT_HOOK_INSTALLED:
        return
    _IMPORT_HOOK = _SentinelImportFinder()
    import sys
...
def patch_runtime(guard: SentinelGuard) -> None:
    global _PATCHED
    with _PATCH_LOCK:
...
def _patch_module_for_name(fullname: str, guard: SentinelGuard) -> None:
    root = fullname.split('.', 1)[0]
    if root == 'requests':
        _patch_requests(guard)
    elif root == 'httpx':
...
def _is_control_plane_request(url: str | None, guard: SentinelGuard) -> bool:
    if not url:
        return False
    try:
        ...
        return False
def _patch_requests(guard: SentinelGuard) -> None:
    try:
        import requests
    except Exception:
        return
    ...
    def wrapper(self, method: str, url: str, *args: Any, **kwargs: Any):
        ...
        if result and result.blocked and current.mode == 'enforce':
            raise SentinelBlockedError(f'HTTP request to {url} blocked', result.decision)
        try:
...
def _patch_httpx(guard: SentinelGuard) -> None:
    ...
    if key not in _ORIGINALS:
        ...
        def send_wrapper(self, request: httpx.Request, *args: Any, **kwargs: Any):
            ...
            if result and result.blocked and current.mode == 'enforce':
                raise SentinelBlockedError(f'HTTPX request to {request.url} blocked', result.decision)
            try:
...
def _patch_openai(guard: SentinelGuard) -> None:
    try:
        from openai.resources.responses.responses import Responses
    except Exception:
        Responses = None
    if Responses is not None:
        ...
        if key not in _ORIGINALS:
            ...
            def wrapper(self, *args: Any, **kwargs: Any):
                ...
                if result and result.blocked and current.mode == 'enforce':
                    raise SentinelBlockedError('OpenAI response call blocked', result.decision)
    ...
    if Completions is not None:
        ...
        if key not in _ORIGINALS:
            ...
            def wrapper(self, *args: Any, **kwargs: Any):
                ...
                if result and result.blocked and current.mode == 'enforce':
                    raise SentinelBlockedError('OpenAI chat completion blocked', result.decision)
                response = _ORIGINALS[key](self, *args, **kwargs)
                if result:
                    current.record_result(action=result.action, decision=result.decision, input_payload=payload, output_payload={'provider': 'openai', 'object': getattr(response, 'object', None)})
                return response
...
def _patch_anthropic(guard: SentinelGuard) -> None:
    try:
        import anthropic
    ...
    if Messages is not None:
        ...
        if key not in _ORIGINALS:
            ...
            def wrapper(self, *args: Any, **kwargs: Any):
                ...
                if result and result.blocked and current.mode == 'enforce':
                    raise SentinelBlockedError('Anthropic message blocked', result.decision)
                response = _ORIGINALS[key](self, *args, **kwargs)
                if result:
...
def _patch_subprocess(guard: SentinelGuard) -> None:
    ...
    if key not in _ORIGINALS:
        ...
        class GuardedPopen(subprocess.Popen):
            def __init__(self, args, *pargs, **kwargs):
                ...
                if result and result.blocked and current.mode == 'enforce':
                    raise SentinelBlockedError('Subprocess execution blocked', result.decision)
...
def protect(**kwargs: Any) -> SentinelGuard:
    guard = SentinelGuard(**kwargs).activate()
