...
public final class SentinelClient {
    ...
    public SentinelClient(SentinelConfig config) {
        this.config = config;
    ...
    public SentinelGuardResult guard(Map<String, Object> action, Object payload) throws IOException, InterruptedException {
        ensureCredentials();
        ...
        SentinelGuardResult result = new SentinelGuardResult(
            new SentinelDecision(String.valueOf(decisionRaw.getOrDefault("action", "allow")), String.valueOf(decisionRaw.getOrDefault("reason", "")), decisionRaw),
            actionRaw,
            data.get("event_id") instanceof Number ? ((Number) data.get("event_id")).intValue() : null
        );
        if (resp.statusCode() == 403) {
            throw new SentinelBlockedException(String.valueOf(data.getOrDefault("detail", "blocked by Sentinel")), result);
        }
        if (resp.statusCode() / 100 != 2) {
            throw new IOException("Sentinel guard failed with status " + resp.statusCode() + ": " + resp.body());
        }
        return result;
    }
}
