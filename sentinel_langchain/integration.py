...
class SentinelCallbackHandler(_LangChainBaseCallbackHandler):
    def __init__(
        self,
        guard: SentinelGuard | None = None,
        *,
...
def _guard_tool_call(tool: Any, *, guard: SentinelGuard | None, agent_name: str, workflow_id: str | None, framework: str, extra_metadata: dict[str, Any] | None, payload: dict[str, Any]) -> GuardResult | None:
...
def _record_tool_result(guard: SentinelGuard | None, result: GuardResult | None, *, input_payload: dict[str, Any], output_payload: Any = None, error: str | None = None) -> None:
    effective_guard = guard or current_guard()
...
def _maybe_block(guard: SentinelGuard | None, result: GuardResult | None, tool_name: str) -> None:
    effective_guard = guard or current_guard()
    if result and result.blocked and effective_guard and effective_guard.mode == 'enforce':
        raise SentinelBlockedError(f'LangChain tool {tool_name} blocked', result.decision)
...
def instrument_tool(
    ...
    *,
    ...
    if getattr(tool, '__sentinel_langchain_protected__', False):
        return tool
    ...
    setattr(tool, '__sentinel_langchain_originals__', originals)
    setattr(tool, '__sentinel_langchain_protected__', True)
    return tool
...
def protect_agent(
    ...
    *,
    ...
    callbacks.append(SentinelCallbackHandler(guard=guard, agent_name=agent_name, workflow_id=workflow_id, framework=framework))
    try:
...
def create_protected_agent(
    *,
    ...
    callback = SentinelCallbackHandler(
        guard=guard,
        agent_name=agent_name,
        workflow_id=workflow_id,
        framework=framework,
