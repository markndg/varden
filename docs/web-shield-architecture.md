...
| 4 | Browser extension (MV3) | **Functional** (page-world wrapping via native `world: "MAIN"` content script + a per-frame nonce/sequence/handshake-validated `MessageChannel` bridge — see `docs/web-shield-hardening-review.md` #2 — badge, popup, options, local fallback, offline queue). The channel-validation logic (`extension/src/protocol.js`) has Node-based unit tests (`extension/test/protocol.test.js`) covering nonce/sequence/handshake/payload-limit adversarial cases. **No automated real-browser tests** — no headless-Chrome/Playwright harness was set up in this environment, so end-to-end page/DOM behaviour (wrapper replacement races, `Object.defineProperty` edge cases, multi-frame injection ordering) is verified by manual code review only, not executed. This remains the single biggest test-coverage gap in the whole feature — see limitations doc. |
