...
class AppConfig:
    ...
    auth_db_path: str = "sentinel_auth.db"
    ...
    def from_env(cls) -> "AppConfig":
        ...
        return cls(
            ...
            auth_db_path=os.getenv("SENTINEL_AUTH_DB_PATH", "sentinel_auth.db"),
            policy_file=os.getenv("SENTINEL_POLICY_FILE", "policy.json"),
            signing_secret=os.getenv("SENTINEL_SIGNING_SECRET", "change-me"),
            host=os.getenv("SENTINEL_HOST", "127.0.0.1"),
            port=int(os.getenv("SENTINEL_PORT", "8000")),
            queue_backend=os.getenv("SENTINEL_QUEUE_BACKEND", "sqlite"),
            redis_url=os.getenv("SENTINEL_REDIS_URL", "redis://localhost:6379/0"),
            worker_poll_interval=float(os.getenv("SENTINEL_WORKER_POLL_INTERVAL", "1.0")),
            worker_concurrency=int(os.getenv("SENTINEL_WORKER_CONCURRENCY", "1")),
            auth_mode=os.getenv("SENTINEL_AUTH_MODE", "local"),
            oidc_issuer=os.getenv("SENTINEL_OIDC_ISSUER"),
            oidc_jwks_url=os.getenv("SENTINEL_OIDC_JWKS_URL"),
            oidc_audience=os.getenv("SENTINEL_OIDC_AUDIENCE"),
            oidc_introspection_url=os.getenv("SENTINEL_OIDC_INTROSPECTION_URL"),
            route_sensitive_to_local=os.getenv("SENTINEL_ROUTE_SENSITIVE_LOCAL", "false").lower() == "true",
            blaze_command=blaze_cmd,
            rate_limit_per_minute=int(os.getenv("SENTINEL_RATE_LIMIT_PER_MINUTE", "600")),
            read_rate_limit_per_minute=int(os.getenv("SENTINEL_READ_RATE_LIMIT_PER_MINUTE", os.getenv("SENTINEL_RATE_LIMIT_PER_MINUTE", "2400"))),
            write_rate_limit_per_minute=int(os.getenv("SENTINEL_WRITE_RATE_LIMIT_PER_MINUTE", "600")),
    ...
    def validate(self) -> list[str]:
        ...
        if self.auth_mode == "jwt" and not self.oidc_jwks_url:
            errors.append("jwt auth mode requires SENTINEL_OIDC_JWKS_URL")
